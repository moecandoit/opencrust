...
async fn test_anthropic_system_message_error() -> Result<()> {
    ...
    if let Err(opencrust_common::Error::Agent(msg)) = result {
        assert_eq!(
            msg,
            "System messages should be passed via the `system` field, not in `messages`"
        );
    } else {
...
async fn test_anthropic_stream() -> Result<()> {
    ...
    assert!(
        has_message_start,
        "Missing MessageStart with input_tokens=10"
    );

    let has_hello = chunks.iter().any(|c| match c {
        LlmStreamResponse::ContentBlockDelta {
            delta: opencrust_agents::providers::ContentBlockDelta::Text { text },
            ..
        } => text == "Hello",
        _ => false,
    });
